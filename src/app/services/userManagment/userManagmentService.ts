...
export class AuthenticationService {
    ...
    login(username: string, password: string) {
        ...
        loginRequest.TenantKey = environment.ApplicationKey;
        let objectToSend = JSON.stringify(loginRequest);
        const header = new HttpHeaders({
            'Content-Type': 'application/json'
        });
        return this.http.post<LoginResponse>(`${environment.apiUrl}/Auth/login`, objectToSend , {headers:header})
